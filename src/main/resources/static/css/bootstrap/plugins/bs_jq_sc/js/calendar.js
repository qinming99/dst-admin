...
(function($) {
		...
		if(where == 'next') {
			switch(this.options.view) {
				...
				case 'month':
					...
					$("#month").val(to.start.getMonthFormatted());
					if (month + 1 == 12){
						$("#year").val(to.start.getFullYear());
					}
					break;
				case 'week':
			...
			case 'string':
				if(source.length) {
					loader = function() {
						...
						var qq = {};
						var param = $.extend(params,q == null ? qq : q);
						$.ajax({
							url: buildEventsUrl(source, param),
							dataType: 'json',
							type: 'GET',
							async: false
						}).done(function(json) {
	...
	Calendar.prototype._templatePath = function(name) {
		if(typeof this.options.tmpl_path == 'function') {
			...
			return this.options.tmpl_path + name + '.ftl';
		}
	};
				...
				modal.off('show.bs.modal')
					...
					.on('show.bs.modal', function() {
						...
						switch(self.options.modal_type) {
							...
							case "ajax":
								$.ajax({
									url: url, dataType: "ftl", async: false, success: function(data) {
