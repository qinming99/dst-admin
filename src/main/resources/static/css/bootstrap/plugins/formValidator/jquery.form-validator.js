...
(function ($) {
  ...
  $.formUtils.addValidator({
    ...
    validatorFunction: function (url) {
      ...
      //var urlFilter = /^(https?|ftp):\/\/((((\w|-|\.|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])(\w|-|\.|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])(\w|-|\.|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/(((\w|-|\.|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/((\w|-|\.|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|\[|\]|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#(((\w|-|\.|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i;
        ...
        let urlFilter = new RegExp(strRegex, 'i');//i不区分大小写
      //   if (urlFilter.test(url)) {
      //       let domain = url.split('://')[1], domainSlashPos = domain.indexOf('/');
      //       if (domainSlashPos > -1) {
        ...
        if (allowing.indexOf('range') > -1) {
          ...
          allowsRange = true;
            if (val < begin || val > end) {
                this.errorMessage = "值范围必须在[" + begin + '-' + end + ']之间';
  ...
  $.formUtils.addValidator({
    ...
    /**
     * 验证字符包括中文除指定字符外不含特殊字符
     */
    $.formUtils.addValidator({
        name: 'letternumeric',
        validatorFunction: function (val, $el, conf, language) {
            var patternStart = '^([a-zA-Z0-9\u4e00-\u9fa5',
                patternEnd = ']+)$',
                additionalChars = $el.valAttr('allowing'),
                pattern = '';
            ...
            if (additionalChars) {
                ...
                this.errorMessage = language.badLetterNumeric + language.badLetterNumericExtra + extra;
            } else {
                pattern = patternStart + patternEnd;
                this.errorMessage = language.badLetterNumeric;
            }
